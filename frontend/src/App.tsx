  ...
  return (
    <main className="screen app-screen">
      ...
      <nav className="nav-grid">
        {(["dashboard", "pain", "diary", "cbt", "dbt", "chat", "settings"] as const).map((item) => (
          <button
            key={item}
            className={nav === item ? "active" : ""}
            onClick={() => setNav(item)}
            disabled={item === "cbt" || item === "dbt"}
          >
            {item === "cbt" ? "CBT" : item === "dbt" ? "DBT" : item}
          </button>
        ))}
      </nav>
      ...
      {nav === "cbt" && (
        <section className="panel"><h2>CBT Thought Response</h2><p className="hint">Coming soon — this entry form is not yet implemented.</p></section>
      )}

      {nav === "dbt" && (
        <section className="panel"><h2>DBT Distress Tolerance</h2><p className="hint">Coming soon — this entry form is not yet implemented.</p></section>
      )}
